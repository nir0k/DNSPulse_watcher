...
func loginHandler(w http.ResponseWriter, r *http.Request, conf sqldb.WebServerConfiguration) {
    if r.Method == "GET" {
        // Display login form
        fmt.Fprintf(w, `
        <html>
        ...
        if !CheckCredentials(username, password, conf) {
...
func authMiddleware(conf sqldb.WebServerConfiguration, handler http.HandlerFunc) http.HandlerFunc {
    return func(w http.ResponseWriter, r *http.Request) {
        ...
        if shouldRefreshToken(token) {
            ...
            setSessionCookie(w, newTokenString, conf)
        }
...
func setSessionCookie(w http.ResponseWriter, token string, conf sqldb.WebServerConfiguration) {
    http.SetCookie(w, &http.Cookie{
        Name:   "session_token",
        Value:  token,
        Path:   "/",
        // MaxAge: 300, // Adjust as per requirement
        MaxAge: conf.SesionTimeout,
    })
}
